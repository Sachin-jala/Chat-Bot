...
from fastapi.responses import HTNLResponse
...
from app.utils import save_uploadd_file
...
async def chat_rest(payload: dict):
    ...
    if user_id not in conversations:
        conversations[user_id] = [SYSTEM_PROMPT.copy()]
    conversations[user_id].append({'role': 'user', 'content': message})

    if use_rag:
        embed = (await embed_text([message], model=MODEL_EMBED))[0]
        for _, md in RAG.search(embed, top_k=RAG_TOP_K):
            conversations[user_id].append({'role': 'system', 'content': f"Source: {md['source']}\n{md['text']}"})

    reply = await create_chat_completion(conversations[user_id], model=MODEL_CHAT)
...
async def Websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            async for chunk in create_chat_completion_stream(conversations[user_id], model=MODEL_CHAT):
